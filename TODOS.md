...
> 마지막 업데이트: 2026-04-22 (iPad 레이아웃 아키텍처 리뷰 반영)
...
| 35 | 커스텀 컬러 피커 (hex 직접 입력, Pro 전용) | P3 | 🗓️ 보류 |
| 36 | 시간대별 자동 테마 전환 (위치/시간 기반) | P3 | 🗓️ 보류 |
| 37 | 폰트 마켓플레이스 (추가 폰트 선택 구매) | P3 | 🗓️ 보류 |
| 38 | 사이드바 navigate vs replace 정책 결정 + 뒤로가기 행동 테스트 | P2 | 🗓️ 보류 |
| 39 | orientation lock + splash screen 타이밍 선애개선 (앱 시작 landscape 깜박임 방지) | P2 | 🗓️ 보류 |
...
| 2026-04-11 | 집중 모드 = Pro 전용 | 페이월 레버 + 복잡도 비용 정당화 |
| 2026-04-11 | 테마 표시 이름 = 한국어 무드 이름 (묵상/여명/시편/광야/저녁/실라) | 앱 정체성 + 한국어 독자 감성 |
| 2026-04-11 | 폰트 로딩 = @expo-google-fonts 패키지 | Managed workflow 완전 호환 |
| 2026-04-11 | 무료 AI 사용: 3회/일 (완전 차단 → KV 카운터로 교체) | 무료 전환율 위해 barrier 낮춤 |
...
**WHY:** 읽기가 앱의 근본. 기본값 품질이 설정 수보다 중요. 기본 테마 개선이 90% 유저에게 즉각 임팩트.

**구현 순서:**
1. `useReaderSettings.ts` — 6종 테마, 5종 폰트, letterSpacing, horizontalMargin, keepAwake, hideVerseNumbers, focusMode 확장 + 마이그레이션 ('dark'→'midnight', 'light'→'daylight')
2. `READER_THEMES` 색상 테이블 6종 (묵상/여명/시편/광야/저녁/실라)
3. 설정 시트 UI 재설계 (테마 스와이프 카드 + 슬라이더)
4. `@expo-google-fonts/nanum-myeongjo` + `@expo-google-fonts/nanum-gothic` + `@expo-google-fonts/nanum-square-round` 설치
5. `expo-keep-awake` 설치 + keepAwake 토글 연결
6. 구절번호 숨기기 토글
7. 집중 모드 (FlatList onViewableItemsChanged, Pro 전용)

**테마 이름:** 코드값(midnight 등) 변경 금지 — 표시 이름만 한국어 무드 이름 사용.
**마이그레이션:** load 시 old → new 자동 변환, fallback = 'midnight'.

---

## P3: 커스텀 컬러 피커 — #35

**WHY:** 고급 Pro 기능. 지금은 큐레이션 테마로 충분.
**조건:** 읽기 UX 고도화 완료 + 유저 피드백에서 요청 확인 후 착수.
**구현:** hex 직접 입력 + Pro 전용 paywall 연결.

---

## P3: 시간대별 자동 테마 전환 — #36

**WHY:** 아침/저녁 자동으로 테마 바꾸는 "스마트" 기능.
**보류 이유:** 복잡도 대비 효과 불확실. 큐레이션 테마 만족도 확인 후 재검토.
**구현 시:** 위치/시간 기반 일출·일몰 API + 사용자 선호 테마 2개(낮/밤) 설정.

---

## P3: 폰트 마켓플레이스 — #37

**WHY:** 폰트 선택의 장기 수익화 가능성.
**보류 이유:** 5종으로 충분한지 먼저 검증. MAU 확보 후 고려.
